...
child_body([token(scope_e, '}') | T], T, child(Acc), Acc).
...
computed_userset_body([token(scope_e, '}') | T], T, computed_userset(Acc), Acc).
...
relation_body([token(scope_e, '}') | T], T, relation(Acc), Acc).
...
rewrite_body([token(scope_e, '}') | T], T, rewrite(Acc), Acc).
...
expression_body([token(scope_e, '}') | T], T, Acc, Acc). % careful, returns a list only, type is wrapped in relevant expression/4.
...
tuple_to_userset_body([token(scope_e, '}') | T], T, tupleset_to_userset(Acc), Acc).
...
tupleset_body([token(scope_e, '}') | T], T, tupleset(Acc), Acc).
